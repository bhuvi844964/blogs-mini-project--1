...
const createBlog = async function (req, res) {
  try {
    ...
    if (!data.title || data.title == "")
      return res
        .status(400)
        .send({ Status: false, message: "Please provide title of the blog " });

    if (!data.body || data.body == "")
      return res
        .status(400)
        .send({ Status: false, message: "Please provide body of the blog " });

    if (!data.authorId || data.authorId == "")
      return res
        .status(400)
        .send({ Status: false, message: "Please provide authorId " });
    else data.authorId = data.authorId.trim();

    if (!data.category || data.category == "")
      return res
        .status(400)
        .send({ Status: false, message: "Please provide blog category " });
...
const getBlogs = async function (req, res) {
  try {
    ...
    if (arr.length > 0) {
      ...
      res
        .status(404)
        .send({ status: false, message: "No such blog is found " });
    }
  } catch (error) {
    res.status(500).send({ status: false, error: error.message });
  }
...
const update = async function (req, res) {
  try {
    ...
    if (!mongoose.isValidObjectId(blogId))
      return res
        .status(400)
        .send({ Status: false, message: "Please enter valid blogId " });
...
const deleteByBlogId = async function (req, res) {
  try {
    ...
    if (blog.isDeleted) {
      return res
        .status(200)
        .send({ success: false, message: "Blog already deleted" });
    }
...
const deleteByQuery = async function (req, res) {
  try {
    ...
    if (!data)
      return res
        .status(404)
        .send({ status: false, msg: "No Record Found or invalid Id " });
